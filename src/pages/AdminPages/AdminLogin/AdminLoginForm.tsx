    ...
    <Stack minH={'100vh'} direction={{ base: 'column', md: 'row' }}>
        <Flex p={8} flex={1} align={'center'} justify={'center'}>
          {adminAcc ? 
            ...
            <Link to='/admin'>
              <span className='bg-sky-600 text-white p-3 rounded-full'>Về trang admin</span>
            </Link>
          </div> : 
            <Stack spacing={4} w={'full'} maxW={'md'}>
              <Heading fontSize={'4xl'} className='text-sky-600'> Admin Cake</Heading>
              <form onSubmit={handleSubmit(onSubmit)} className='text-maintext'>
                  <FormControl id="email">
                    <FormLabel>Tài khoản</FormLabel>
                    <Input {...register('adminAccount',{required:true,})} onChange={handleInputData} type="text"  />
                  ...
                  <FormControl id="password" mt={5}>
                    ...
                    <Input {...register('adminPassword',{required:true,minLength:6,})} onChange={handleInputData} type="password"  />
