      ...
      <div className='grid grid-cols-3 gap-6 lg:grid-cols-2 md:grid-cols-1'>
        {news.map(item =>(
          <div className='border-[1px] rounded-[10px] p-3 shadow-md flex flex-col' key={item._id}>
            ...
            <div className=''>
              <Link to={`/tin-tuc/${item.newsTitle}`} className='w-full py-2 cursor-pointer text-indigo-700 text-center block font-semibold rounded-[10px] hover:brightness-90 duration-200'>
                Xem thêm
              </Link>
            </div>
       </div>
     ...
     <nav className='text-[16px] cursor-pointer'>
      <ul className="inline-flex items-center -space-x-px">
        <li onClick={()=>changePage(pageNum-1)} className='px-1'>
          <span className="block px-3 py-2 ml-0 text-indigo-700 hover:text-indigo-700 duration-200" style={pageNum === 1 ? {cursor:'not-allowed'} : {}}>
          <i className="fa-solid fa-chevron-left"></i>
          </span>
        </li>
        {[...Array(pageSum)].map((item,index)=>(
          <li onClick={()=>changePage(index+1)} key={index} className='px-1'>
          <span className="px-3 py-1 text-indigo-700 hover:text-indigo-700 duration-200" 
          style={pageNum === index +1 ? {color: 'white',backgroundColor:'#00aaee',borderRadius:'5px'} : {}}>{index+1}</span>
        </li>
        ))}
        <li onClick={()=>changePage(pageNum+1)} className='px-1'>
          <span className="block px-3 py-2 text-indigo-700 hover:text-indigo-700 duration-200 " style={pageNum === pageSum  ? {cursor:'not-allowed'} : {}}>
