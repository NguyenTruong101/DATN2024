...
const Cart = () => {
  ...
  return (
    <section className='max-w-[1200px] m-auto lg:px-5'>
      <h1 className='text-[40px] md:text-[30px] sm:text-[25px] py-5 overflow-hidden relative font-semibold text-sky-600'>Giỏ hàng</h1>
      <div className='flex justify-end'>
        <span className='text-[16px] cursor-pointer' onClick={backToPrev}><i className="fa-solid fa-arrow-left"></i> Quay lại</span>
      </div>
        {cartList.length === 0 ? 
          <span className='text-[20px] font-semibold text-red-600'>Hiện tại chưa có sản phẩm nào trong giỏ hàng</span> : 
          <div>
           ...
           <div key={item._id}>
             <div className='flex text-maintext items-center justify-between py-5 md:flex-col md:px-10'>
              ...
              <NumberInput value={item.qty} onChange={handleQuantity} defaultValue={1} size='md' maxW={32} min={1} max={item.quantity} focusBorderColor='#00aaee'>
                <NumberInputField />
                <NumberInputStepper>
              ...
              <span onClick={()=>removeLocalProd(item)} className='text-[16px] text-sky-600 hover:underline cursor-pointer ml-[60px] md:ml-0 md:py-3'>Xóa</span>
            </div>
            <hr/>
          </div>
        ))}
        <div className='flex flex-col items-end text-maintext mt-5'>
          <span onClick={removeAllLocalProd} className='font-semibold text-[16px] text-sky-600 hover:underline cursor-pointer'>Xóa tất cả</span>
          <span className='text-[20px] py-5 font-semibold ml-10 md:ml-0 md:pb-3'>Tổng tiền: {formatVND(calculateTotal())}</span>
          <Link to='/thanh-toan'>
            <button className='w-[180px] mt-3 py-2 bg-sky-600 text-white font-semibold rounded-[10px] hover:brightness-90 duration-200'>Mua hàng</button>
          </Link>
        </div> 
       </div>  
      }
