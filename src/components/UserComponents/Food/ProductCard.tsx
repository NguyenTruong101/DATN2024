  ...
  return (
    <>
      {productList?.map((item,index)=>(
        ...
        <span className='text-[25px] text-sky-600 font-bold py-2 block'>{formatVND(item.prodPrice)}</span>
          <button onClick={()=>handleAddToCart(item)} className='w-full py-2 bg-sky-600 text-white font-semibold rounded-[10px] hover:brightness-90 duration-200'>
            Thêm vào giỏ hàng
          </button>
        </div>
      </div>
