...
const HomePromotion = () => {
  ...
  return (
    <section className='mt-10 lg:px-5'>
      <h1 className='text-[40px] md:text-[30px] sm:text-[25px] py-5 overflow-hidden relative font-semibold text-sky-600 '>Khuyến mãi đặc biệt</h1>
      <Swiper
      ...
      {proFood?.filter(item=>item.saleOff === 'khuyen-mai').map((item,index)=>(
        ...
        <Link to={`/thuc-don/${item.prodType}/${item.prodName}`}>
          <div className='relative pt-[100%] lg:pt-[80%] md:pt-[60%]'>
            <img className='absolute top-0 left-0 h-full w-full object-cover rounded-[10px] cursor-pointer' src={item.prodImg} alt={item.prodName} />
          </div>
        </Link>
        ...
        <span className='text-[25px] text-[#00aaee] font-bold py-2 block'>{formatVND(item.prodPrice)}</span>
          <button onClick={()=>handleAddToCart(item)} className='w-full py-2 bg-[#00aaee] text-white font-semibold rounded-[10px] hover:brightness-90 duration-200'>
            Thêm vào giỏ hàng
          </button>
        </div>
      </div>
